...
approved = [
    r for r in recommendations if r["keyword"] not in (REJECT_KEYWORDS | HOLD_KEYWORDS)
]
...
if rejected:
    ...
    for r in rejected:
        print(
            f"  REJECT: {r['keyword']} - {r['recommended_weight']:.1f}x (mechanical artifact)"
        )
    print()
...
if held:
    ...
    for r in held:
        print(
            f"  HOLD: {r['keyword']} - {r['recommended_weight']:.1f}x ({r.get('confidence', 0):.0%}
                conf)"
        )
    print()
...
for i, r in enumerate(approved, 1):
    print(
        f"  {i}. {r['keyword']} -> {r['recommended_weight']:.1f}x ({r.get('confidence', 0):.0%} conf
            )"
    )
...
for rec in approved:
    ...
    if keyword not in keyword_stats:
        ...
        keyword_stats[keyword]["last_updated"] = (
...
report_data = {
    "applied_date": datetime.now(timezone.utc).isoformat() + "Z",
