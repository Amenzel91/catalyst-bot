...
import os, sqlite3, pathlib, json, time, typing as T
...
def insert_screener_rows(conn: sqlite3.Connection, screen_key: str, rows: list[dict]) -> int:
    ts = now_ts()
    cur = conn.cursor()
    n = 0
    for r in rows:
        ...
        relvol = _to_float(r.get("Rel Volume") or r.get("RelVolume") or r.get("relvolume"))
        cur.execute(
            "INSERT INTO finviz_screener_snapshots(captured_at,screen_key,ticker,price,change,relvolume,raw_json) "
            "VALUES (?,?,?,?,?,?,?)",
...
def insert_filings(conn: sqlite3.Connection, ticker: str, rows: list[dict]) -> int:
    ...
    for r in rows:
        ...
        fdate = (r.get("Filing Date") or r.get("filingDate") or r.get("Date") or "").strip() or None
        title = (r.get("Title") or r.get("title") or "").strip() or None
        link = (r.get("Link") or r.get("link") or "").strip() or None
        cur.execute(
            "INSERT OR IGNORE INTO finviz_filings(captured_at,ticker,form,filing_date,title,link,raw_json) "
            "VALUES (?,?,?,?,?,?,?)",
