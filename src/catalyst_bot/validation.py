...
def validate_ticker(ticker: str, allow_empty: bool = False) -> Optional[str]:
    ...
    if len(ticker_str) > MAX_TICKER_LENGTH:
        log.warning("ticker_validation_failed reason=too_long length=%d ticker=%s",
                   len(ticker_str), ticker_str[:20])
        return None

    # Check format - must match pattern
    if not TICKER_PATTERN.match(ticker_str):
        log.warning("ticker_validation_failed reason=invalid_format ticker=%s",
                   ticker_str[:20])
        return None
    ...
    if '/' in ticker_normalized:
        ...
        if len(parts) != 2 or not all(p and p.isalpha() and len(p) == 3 for p in parts):
            log.warning("ticker_validation_failed reason=invalid_forex_format ticker=%s",
                       ticker_normalized)
            return None
    ...
    for pattern in dangerous_patterns:
        ...
        if pattern in ticker_normalized:
            log.warning("ticker_validation_failed reason=dangerous_pattern pattern=%s ticker=%s",
                       pattern, ticker_normalized[:20])
            return None
...
def sanitize_filename(filename: str, max_length: int = 255) -> Optional[str]:
    ...
    if len(name) > max_length or len(name) == 0:
        log.warning("filename_validation_failed length=%d name=%s",
                   len(name), name[:50] if name else '')
        return None
