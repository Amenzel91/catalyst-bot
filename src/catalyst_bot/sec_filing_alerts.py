...
def _calculate_dilution_percentage(new_shares: int, ticker: str) -> Optional[float]:
    """
    Calculate dilution percentage from new share issuance.

    Attempts to fetch current shares outstanding from market data API
    and calculate dilution as: (new_shares / current_shares) * 100
    ...
    -------
    float or None
        Dilution percentage if calculable, None otherwise

    Notes
    -----
    This is a best-effort calculation. Returns None if:
    - Market data unavailable
    - API request fails
    - Shares outstanding not found
    """
    try:
        # Try to get current shares outstanding from market module
        from .market import get_market_data

        market_data = get_market_data(ticker)
        if market_data and hasattr(market_data, "shares_outstanding"):
            shares_outstanding = market_data.shares_outstanding
            if shares_outstanding and shares_outstanding > 0:
                dilution_pct = (new_shares / shares_outstanding) * 100
                return dilution_pct
    except Exception as e:
        log.debug(f"Could not calculate dilution for {ticker}: {e}")

    return None


def _format_filing_items(
    filing_section,
    numeric_metrics=None,
) -> str:
    """
    Format filing items with bullet points and actionable data.

    Transforms filing metadata into user-friendly, actionable format:
        Before: Item 2.01: Completion of Acquisition or Disposition of Assets
        After:  • Completion of Acquisition or Disposition of Assets (Item 2.01)
                  └ Deal Size: $2.9M
    ...
    numeric_metrics : NumericMetrics, optional
        Pre-extracted metrics for additional context

    Returns
    -------
    str
        Formatted filing items section with bullets and actionable data
    """
    ...
    item_display = f"• {filing_section.item_title}"
    if filing_section.item_code and filing_section.item_code != "N/A":
        item_display += f" (Item {filing_section.item_code})"

    lines.append(item_display)

    # Add actionable data based on item type
    details = []

    # Item 2.01: Completion of Acquisition - show deal size
    if filing_section.item_code == "2.01":
        if hasattr(filing_section, "deal_size_usd") and filing_section.deal_size_usd:
            if filing_section.deal_size_usd >= 1_000_000:
                deal_str = f"${filing_section.deal_size_usd / 1_000_000:.1f}M"
            else:
                deal_str = f"${filing_section.deal_size_usd:,.0f}"
            details.append(f"Deal Size: {deal_str}")

    # Item 3.02: Unregistered Sales - show share count and dilution
    elif filing_section.item_code == "3.02":
        if hasattr(filing_section, "share_count") and filing_section.share_count:
            share_display = f"{filing_section.share_count:,}"

            # Try to calculate dilution percentage
            ticker = getattr(filing_section, "ticker", "")
            dilution_pct = None
            if ticker:
                dilution_pct = _calculate_dilution_percentage(filing_section.share_count, ticker)

            # Format with dilution if available
            if dilution_pct is not None:
                details.append(f"Shares: {share_display} | {dilution_pct:.1f}% dilution")
            else:
                details.append(f"Shares: {share_display}")

    # Item 1.01: Entry into Material Agreement - show agreement value
    elif filing_section.item_code == "1.01":
        if hasattr(filing_section, "deal_size_usd") and filing_section.deal_size_usd:
            if filing_section.deal_size_usd >= 1_000_000:
                deal_str = f"${filing_section.deal_size_usd / 1_000_000:.1f}M"
            else:
                deal_str = f"${filing_section.deal_size_usd:,.0f}"
            details.append(f"Agreement Value: {deal_str}")

    # Item 1.02: Termination of Material Agreement
    elif filing_section.item_code == "1.02":
        details.append("Agreement terminated")

    # Item 5.02: Leadership changes
    elif filing_section.item_code == "5.02":
        # Note: Extracting specific names requires NLP - this is a limitation
        if hasattr(filing_section, "text"):
            text_lower = filing_section.text.lower()
            if "departure" in text_lower or "resigned" in text_lower:
                details.append("Leadership departure")
            if "appointment" in text_lower or "appointed" in text_lower:
                details.append("New appointment")

    # Add detail lines with tree-style indent
...
def create_sec_filing_embed(
    ...
    Layout:
    - Title: 🚨 {TICKER} | {Filing Type} - {Priority Badge}
    ...
    if priority_score:
        ...
        if priority_score.reasons:
            reasons_text = "\n".join(f"• {r}" for r in priority_score.reasons[:3])
            priority_value += f"\n{reasons_text}"

    _add_field(embed, "Priority", priority_value, inline=False)

    # Add filing items section with actionable data
    filing_items_formatted = _format_filing_items(filing_section, numeric_metrics)
    if filing_items_formatted:
        _add_field(embed, "📋 Filing Items", filing_items_formatted, inline=False)
    ...
    if numeric_metrics:
        ...
        if metrics_parts:
            metrics_value = "\n".join(metrics_parts)
            _add_field(embed, "💰 Key Metrics", metrics_value, inline=False)
    ...
    if guidance_analysis and guidance_analysis.has_guidance:
        ...
        if guidance_parts:
            guidance_value = "\n".join(guidance_parts)
            _add_field(embed, "📈 Forward Guidance", guidance_value, inline=False)
    ...
    if sentiment_output:
        ...
        _add_field(embed, "🎯 Sentiment", sentiment_value, inline=False)

    # Add keywords if available
    if keywords:
        keywords_display = ", ".join(f"`{kw}`" for kw in keywords[:10])
        _add_field(embed, "🏷️ Keywords", keywords_display, inline=False)
...
def create_sec_filing_buttons(
    ...
    Buttons:
    1. View Filing 📄 - Link to SEC.gov filing
    2. Dig Deeper 🔍 - Trigger RAG Q&A (requires interaction endpoint)
    3. Generate Chart 📊 - Generate price chart
...
async def send_daily_digest(
    ...
    try:
        ...
        embed = {
            "title": "📰 Daily SEC Filings Digest",
            "description": f"Summary of {len(filings)} filings from {len(by_ticker)} companies",
            "color": 0x3498DB,  # Blue
            "timestamp": datetime.now(timezone.utc).isoformat(),
            "fields": [],
            "footer": {"text": "Medium/Low priority filings • View individual filings for details"},
        }
