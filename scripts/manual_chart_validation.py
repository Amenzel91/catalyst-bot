...
from catalyst_bot.chart_cache import get_cache  # noqa: E402
from catalyst_bot.charts_advanced import (  # noqa: E402
    TIMEFRAME_CONFIG,
...
def test_single_chart(ticker: str, timeframe: str = "1D"):
    ...
    chart_path = generate_multi_panel_chart(
        ticker=ticker, timeframe=timeframe, style="dark"
    )
    ...
    if chart_path and chart_path.exists():
        ...
        if cached_path:
            print("[OK] Chart found in cache!")
            print(f"   Cache key: {ticker}_{timeframe}")
        else:
            print("[WARN] Chart not in cache, adding...")
            cache.put(ticker, timeframe, chart_path)
            print("[OK] Added to cache")
...
def main():
    ...
    parser.add_argument(
        "--all-timeframes", action="store_true", help="Generate all timeframes"
    )
    parser.add_argument(
        "--test-cache", action="store_true", help="Test cache operations"
    )
